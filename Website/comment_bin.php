   ...
   $database = "project_management";
	...
	$conn = mysqli_connect($servername, $username, $password,$database);
		...
		//echo $Admin_id, " = ";
	 	...
	 	$sql = mysqli_query($conn, "SELECT id FROM comments WHERE show_comment	 = '1' AND delete_id = '$Admin_id'");
		...
		if(isset($_POST["restore"])){
			...
			$CId = $_POST['restore']; 
			//echo $SId ;
			$sql= "UPDATE comments SET show_comment ='0' WHERE id = '$CId'";
...
<body>
<pre> <h2><a href="AdminRecycleBin.php" style="text-decoration:none"><font color="#1A2238" face="Calibri"> [MiniCourse]</font></a> <a href="comment_bin.php" style="text-decoration:none">	<font color="#2568FB" face="Calibri"> [Comments]</font></a></a> <a href="replay_bin.php" style="text-decoration:none">	<font color="#3f63a1" face="Calibri"> [Replay]</font></a></h2><hr></pre>
...
<?php
		...
		$sql = "SELECT id FROM comments WHERE show_comment = '1' AND delete_id = '$Admin_id' ORDER BY deleted_date DESC ";
		$result = $conn->query($sql);
		while(($c<$Num_row) && ($row = $result->fetch_assoc())){
			$cNo[0] = $row['id'];
	
			$sqlN = $conn->query("SELECT comments.id, name, comment,createdOn,deleted_date FROM comments INNER JOIN member ON comments.memberId = member.id WHERE comments.id  = '$cNo[0]' ");
