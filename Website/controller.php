<?php
    ...
    if (isset($_POST['forgot_password'])) {
        ...
        $emailCheckQuery = "SELECT * FROM member WHERE email = '$email'";
        ...
        if ($emailCheckResult) {
            ...
            if (mysqli_num_rows($emailCheckResult) > 0) {
                ...
                $updateQuery = "UPDATE member SET code = $code WHERE email = '$email'";
...
if(isset($_POST['verifyEmail'])){
    ...
    $verifyQuery = "SELECT * FROM member WHERE code = $OTPverify";
    $runVerifyQuery = mysqli_query($conn, $verifyQuery);
    if($runVerifyQuery){
        if(mysqli_num_rows($runVerifyQuery) > 0){
            $newQuery = "UPDATE member SET code = 0";
...
if(isset($_POST['changePassword'])){
    ...
    if (strlen($_POST['password']) < 8) {
        ...
        if ($_POST['password'] != $_POST['confirmPassword']) {
            ...
            $updatePassword = "UPDATE member SET pass = '$password' WHERE email = '$email'";
            $updatePass = mysqli_query($conn, $updatePassword) or die("Query Failed");
            session_unset($email);
            session_destroy();
            header('location: newSignin.html');
        }
    }
}